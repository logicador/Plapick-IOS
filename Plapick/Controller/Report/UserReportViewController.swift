...
class UserReportViewController: UIViewController {
    ...
    // MARK: Property
    let reportUserRequest = ReportUserRequest()
    var user: User?
    ...
    override func viewDidLoad() {
        ...
        reportUserRequest.delegate = self
    }
    ...
    @objc func submitTapped() {
        ...
        reportUserRequest.fetch(vc: self, paramDict: ["uId": String(user.id), "reason": reason])
    }
}
...
extension UserReportViewController: ReportUserRequestProtocol {
    func response(reportUser status: String) {
        print("[HTTP RES]", reportUserRequest.apiUrl, status)
