...
class UserViewController: UIViewController {
    ...
    func getUser() {
        ...
        getUserRequest.fetch(vc: self, paramDict: ["uId": String(uId)])
    }
    
    func getPosts() {
        guard let uId = self.uId else { return }
        isPostsLoading = true
        getPostsRequest.fetch(vc: self, paramDict: ["mode": "USER", "uId": String(uId), "page": String(postsPage)])
    }
    
    func getPlaces() {
        guard let uId = self.uId else { return }
        getPlacesRequest.fetch(vc: self, paramDict: ["mode": "POSTS", "uId": String(uId)])
    }
...
extension UserViewController: PostsPlaceTVCellProtocol {
...
// MARK: HTTP - GetUser
extension UserViewController: GetUserRequestProtocol {
    func response(user: User?, getUser status: String) {
        ...
        if status == "OK" {
            ...
            navigationItem.rightBarButtonItem = UIBarButtonItem(title: (user.u_is_follow == "Y") ? "팔로우 취소" : "팔로우", style: .plain, target: self, action: #selector(followTapped))
            
            navigationItem.title = user.u_nickname
...
// MARK: HTTP - GetPosts
extension UserViewController: GetPostsRequestProtocol {
...
// MARK: HTTP - GetPlaces
extension UserViewController: GetPlacesRequestProtocol {
    func response(placeList: [Place]?, getPlaces status: String) {
        ...
        if status == "OK" {
            ...
            if placeList.count > 0 {
                ...
                getPostsRequest.fetch(vc: self, paramDict: ["mode": "PLACES", "uId": String(uId), "pIdList": pIdList])
            }
        }
...
// MARK: HTTP - Follow
extension UserViewController: FollowRequestProtocol {
    func response(isFollow: String?, follow status: String) {
        ...
        if status == "OK" {
            ...
            navigationItem.rightBarButtonItem = UIBarButtonItem(title: (isFollow == "Y") ? "팔로우 취소" : "팔로우", style: .plain, target: self, action: #selector(followTapped))
            
            delegate?.follow(uId: uId)
