...
protocol PostsCommentTVCellProtocol {
...
class PostsCommentTVCell: UITableViewCell {
    
    // MARK: Property
    var delegate: PostsCommentTVCellProtocol?
    var postsComment: PostsComment? {
        didSet {
            ...
            reCommentButton.setTitle("답글 \(postsComment.poc_re_comment_cnt)개", for: .normal)
        }
    }
    ...
    @objc func userTapped() {
        guard let postsComment = self.postsComment else { return }
        delegate?.selectUser(uId: postsComment.poc_u_id)
    }
    
    @objc func moreTapped() {
        guard let postsComment = self.postsComment else { return }
        delegate?.more(postsComment: postsComment)
    }
    
    @objc func reCommentTapped() {
        guard let postsComment = self.postsComment else { return }
        delegate?.reComment(postsComment: postsComment)
    }
}
