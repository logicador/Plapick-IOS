...
// MARK: Protocol
protocol SetMyProfileRequestProtocol {
    func response(user: User?, status: String)
}


class SetMyProfileRequest {
    // MARK: Properties
    var delegate: SetMyProfileRequestProtocol?
    let apiUrl = API_URL + "/set/my/profile"
